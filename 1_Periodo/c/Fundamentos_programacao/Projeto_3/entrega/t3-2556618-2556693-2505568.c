...
void retiraRuido(Imagem1C* imagem_tratada);
int tiraMedia(Imagem1C* imagem_tratada, int altura, int largura);
...
int contaVeiculos(Imagem3C* img, Imagem3C* bg, int contagem[N_TIPOS_DE_VEICULOS]){
	...
	Imagem1C* imagem_tratada=subtraiImagem(img, bg);
	int largura_imagem = imagem_tratada->largura;
	int altura_imagem = imagem_tratada->altura;
	...
	for(int i=1; i<(altura_imagem)-1; i++){
    		for(int j=1; j<(largura_imagem)-1; j++){
...
Imagem1C* subtraiImagem(Imagem3C* img, Imagem3C* bg){
	...
	int largura_background = bg->largura;
	int altura_background = bg->altura;
	...
	for(int altura=0; altura<altura_background; altura++){
		
    		for(int largura=0; largura<largura_background; largura++){
	...
	for(int i=altura-1; i<=altura_img; i++){
    		for(int j=largura-1; j<=largura_img; j++){
        		if(j!=largura||i!=altura){
            			media+=imagem_tratada->dados[i][j];
        		}
    		}
	}
	media=media/8;
